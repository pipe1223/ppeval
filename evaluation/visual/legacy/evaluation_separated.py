...
"""
tianhuieval_bridge_multitask_v3.py

Fixes a GT-mapping edge case in v2:

- If a GT file contains per-item "task_type" in gt[] (or any task typing in items),
  v2 only registered it when it contained >1 task types.
- But many GT files are effectively "single-task" and still store task_type per item.
  In that case v2 failed to register the GT under that task when info.task and
  filename [TASK] were missing, leading to "missing_gt".

v3 registers task->GT views from split_by_task() for ANY number of tasks, as long
as the split keys are meaningful (not just {"UNKNOWN": ...}).

Usage is the same as v2:
...
# results reader (prefer user's local result_reader.py)
try:
    from evaluation.visual.legacy.result_reader import load_results  # type: ignore
except Exception:
    from evaluation.visual.legacy.result_reader import load_results  # type: ignore

# GT reader v2 or v3 if user prefers
try:
    from evaluation.visual.legacy.gt_reader import load_gt  # type: ignore
except Exception:
    from evaluation.visual.legacy.gt_reader import load_gt  # type: ignore

from evaluation.visual.legacy.eval_core import const
...
def _to_cls_lists(y_true_raw, y_pred_raw):
    ...
    for gt_items, pred_items in zip(y_true_raw, y_pred_raw):
        gt_label = ""
        if gt_items:
            gt_label = _label_to_multilabel_str(getattr(gt_items[0], "label", gt_items[0]))
        y_true.append(gt_label.lower())
...
def _to_text_lists(y_true_raw, y_pred_raw):
    ...
    for gt_items, pred_items in zip(y_true_raw, y_pred_raw):
        gt_text = _first_text(getattr(gt_items[0], "label", "")) if gt_items else ""
...
def _to_seg_polygons(y_true_raw, y_pred_raw):
    ...
    y_pred = []
    for gt_items, pred_items in zip(y_true_raw, y_pred_raw):
        y_true.append(gt_items_to_polys(gt_items))
        y_pred.append(pred_items_to_polys(pred_items))
    return y_true, y_pred
...
def _evaluate_one_task(*, task_type: str, entries, gt_file, result_key: str, iou: float, strict: bool):
    y_true_raw, y_pred_raw, questions, crops, n_missing = _build_pairs_for_task(
        entries, gt_file, result_key=result_key, strict=strict
    )

    if task_type in const.CLS_TAGS:
        from evaluation.visual.legacy.eval_core.classification import evaluate_classification
        ...
        for s in y_true:
            for lab in [x.strip() for x in s.split(",")]:
                if lab and lab not in classes:
                    classes.append(lab)
        out = evaluate_classification(y_true, y_pred, classes)
        results = {"raw": out}

    elif task_type in const.CAP_TAGS:
        from evaluation.visual.legacy.eval_core.captioning import evaluate_captioning
        ...
        from evaluation.visual.legacy.eval_core.vqa import evaluate_vqa
        y_true, y_pred = _to_text_lists(y_true_raw, y_pred_raw)
        ...
        from evaluation.visual.legacy.eval_core.segmentation import evaluate_segmentation
        ...
        from evaluation.visual.legacy.eval_core.detection_hbb import evaluate_detection
        ...
        try:
            ...
            raise RuntimeError(
                "REG_DET_HBB evaluator does not appear to support in-memory inputs. "
                "The local HBB evaluator could not run with the provided in-memory format."
            ) from e
        ...
        from evaluation.visual.legacy.eval_core.detection_obb import evaluate_detection2
        data = _to_obb_data(y_true_raw, y_pred_raw, questions)
        det = evaluate_detection2(data, pre_box=False, is_box=False, is_obb=True)
        results = det
...
def load_gt_map(*, gt_paths: Optional[Sequence[str]] = None, gt_dir: Optional[str] = None) -> Dict[str, Any]:
    """
    Load GT files into a mapping: task_tag -> GTFile (filtered to that task).

    v3 change:
    - If gf.split_by_task() yields meaningful task keys (even just ONE),
      we register them. This fixes missing_gt when info.task is absent and filename
      lacks [TASK], but per-item task_type exists.
    """
    paths: List[str] = []
    if gt_paths:
    ...
    for p in paths:
        ...
        if isinstance(split, dict) and split:
            # If the only key is UNKNOWN, ignore split and fall back to other methods.
            if not (len(split) == 1 and "UNKNOWN" in split):
                for t, sub in split.items():
                    if t and t != "UNKNOWN":
                        gt_map[str(t)] = sub
                continue  # done with this file

        # Single-task fallback: info.task or filename [TASK]
...
def evaluate_files(
    ...
    *,
    ...
    for tt, entries in groups.items():
        ...
        if gf is None:
            out_by_task[tt] = {
                "info": {"task": tt, "error": "missing_gt", "n_entries": len(entries)},
                "results": {"error": f"No GT found for task={tt}. Provide GT via --gt/--gt-dir and ensure GT has info.task or per-item task_type in gt[]."},
            }
            continue
        out_by_task[tt] = _evaluate_one_task(
            task_type=tt,
            entries=entries,
            gt_file=gf,
            result_key=result_key,
