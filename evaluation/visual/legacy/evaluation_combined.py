...
def evaluate_from_ytrue_ypred(*, task: str, y_true: List[Any], y_pred: List[Any], crops: Optional[List[Any]] = None, questions: Optional[List[Optional[str]]] = None, iou: float = 0.5, dataset: Optional[str] = None, model: Optional[str] = None) -> Dict[str, Any]:
    task = str(task)

    if task in const.SEG_TAGS:
        if crops is None:
            raise ValueError(f"crops is required for task={task} (PIX_SEG/PIX_CHG)")
        class_list = [(_extract_class(q) or "PIPE") for q in questions] if questions else None
        ...
        out = evaluate_classification([str(x).strip().lower() for x in y_true], [str(x).strip().lower() for x in y_pred], class_names)
        ...
        for i in range(len(y_true)):
            data_json.append({"gt": y_true[i], "answer": y_pred[i], "question": (questions[i] if questions and i < len(questions) else None)})
        results_payload = evaluate_detection2(data_json, pre_box=False, is_box=False, is_obb=True)
    else:
        results_payload = {"error": f"Unsupported task: {task}"}

    return {"info": {"task": task, "dataset": dataset, "model": model, "n_eval": len(y_true)}, "results": results_payload}
...
def evaluate_evalfile(ef: EvalFile, *, iou: float = 0.5) -> Dict[str, Any]:
    ...
    if task == "REG_DET_HBB":
        ...
        return evaluate_from_ytrue_ypred(task=task, y_true=y_true_hbb, y_pred=y_pred_hbb, crops=None, questions=[s.question for s in ef.data], iou=iou, dataset=ef.info.dataset, model=ef.info.model)
    if task in ("REG_DET_OBB", "REG_VG"):
        return evaluate_from_ytrue_ypred(task=task, y_true=y_true, y_pred=y_pred, crops=None, questions=[s.question for s in ef.data], iou=iou, dataset=ef.info.dataset, model=ef.info.model)
    return evaluate_from_ytrue_ypred(task=task, y_true=y_true, y_pred=y_pred, crops=crops, questions=[s.question for s in ef.data], iou=iou, dataset=ef.info.dataset, model=ef.info.model)
